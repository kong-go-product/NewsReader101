...
def extract_article_content(link):
    try:
        ...
        content = f"**{title}**" + doc.summary()  # Combine bold title and main content as HTML
...
def clean_text(text):
    ...
    text = re.sub(r'</body>', '\n\n', text)  # Replace closing </body> with double newlines
    text = re.sub(r'<html.*?>', '', text)  # Remove <html> tags 
    text = re.sub(r'</html>', '\n\n', text)  # Replace closing </html> with double newlines
    ...
    cleaned_lines = [re.sub(r'&[a-zA-Z0-9#]+;', '', line) for line in cleaned_lines]  # Remove HTML entities
    cleaned_lines = [re.sub(r'\s+', ' ', line) for line in cleaned_lines]  # Replace multiple spaces with a single space
...
def send_email(recipient,titles):
    ...
    with open(f"news_{datetime.now().strftime('%Y-%m-%d')}.pdf", "rb") as pdf_file:
        ...
        pdf_part.add_header('Content-Disposition', f'attachment; filename=news_{datetime.now().strftime("%Y-%m-%d")}.pdf')
        msg.attach(pdf_part)
...
def convert_txt_to_pdf():
    ...
    column_width = (pdf.w - 20) / 3  # Subtracting 20 for margins, then dividing by 3 columns
    ...
    for line in lines:
        ...
        pdf.multi_cell(column_width, 10, line.encode('latin-1', 'replace').decode('latin-1'))
